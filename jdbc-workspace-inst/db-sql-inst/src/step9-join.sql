/*
	...
	1. INNER JOIN - JOIN 조건에서 값이 일치하는 행을 반환 
		1) equi-join 2) non-equi-join 
		
	2. OUTER JOIN - JOIN 조건에서 한쪽 값이 일치하지 않더라도 행을 반환 
...
/*
 		...
 		2) non-equi join : 조인 테이블 간의 상응하는 컬럼 정보가 없을 때
 					...
 					(사원 테이블과 월급등급테이블은 컬럼에 연관성은 없지만
 					 사원의 월급 등급을 확인하기 위해 non-equi join 이용)
 ...
 */
 ...
 -- 가장 높은 sal 월급을 받는 사원 emp 의 empno,ename,sal,deptno,loc를 조회
 SELECT max(sal) FROM emp;
 SELECT empno FROM emp WHERE sal=5000;
 -- 위 두 sql 을 서브쿼리로 합쳐서 사원번호를 조회한다 
 SELECT empno FROM emp WHERE sal=(
 	SELECT max(sal) FROM emp
 ); 
-- subquery와 join 을 이용해 조회 
...
--		 sal				hisal  , losal   
--     두 테이블의 연관성은 사원의 sal과 salgrade의 최고액hisal 과 최저액losal 사이에 있다
   select * from emp; -- SMITH 의 SAL 은 800 
...
/*
 		...
 		사례 - 
 */
-- SMITH 의 매니저번호(mgr)는 7902이다 
SELECT empno,ename,mgr FROM emp WHERE empno=7369; 
-- SMITH의 매니저 번호 mgr 은 또 다른 사원의 사원번호empno이다 
...
FROM	emp e, emp m
WHERE e.mgr=m.empno;
-- 위 SQL은 INNER JOIN 형태이므로 매니저가 없는 사원은 제외되어 13명이 조회(원래 14명)
-- 매니저가 없는 사원까지 모두 조회하고자 할 때는 OUTER JOIN 을 이용한다 
SELECT e.empno,e.ename,e.mgr,m.empno,m.ename
FROM	emp e, emp m
WHERE e.mgr=m.empno(+);

-- 7369 empno 의 ename과 dname , 매니저명(m.ename) , grade(월급등급) 을 조회 

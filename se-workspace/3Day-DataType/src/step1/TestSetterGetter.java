...
public class TestSetterGetter {
	public static void main(String[] args) {// main ctrl+space로 생성함
		//Person 객체 생성, 참조변수는 p로 한다.
		Person p=new Person();
		//p객체의 name에 아이유를 할당해본다.
		//p.name="아이유";//private 이므로 접근 불가.
		//name에 할당하기 위해 setter인 setName 메서드를 이용한다.
		p.setName("아이유");
		/*
		 ...
		 */
		//위 두 라인을 한라인으로 표현하면 아래와 같다.
		System.out.println(p.getName());
		System.out.println("************************************");
		//아래 코드에 대응되는 Person class의 요소를 코딩
		//1. 인스턴스 변수 2. setter 3. getter
		//p.age=-11; private이므로 접근불가

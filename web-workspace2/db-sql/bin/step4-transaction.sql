/*
	...
	- ROLLBACK : 변경된 모든 내용을 취소하고 이전 상태로 되돌린다.
	...
	ex1) 계좌이체 업무 단위(계좌이체 Transaction) 에서는 
		...
		만약 하나의 작업이라도 문제가 발생할 경우에는 현 작업단위의 모든 작업을 취소해야 한다. → ROLLBACK
		
		이를 위해서는 데이터베이스를 사용하는 프로그램 차원에서 
		COMMIT을 수동모드로 전환해야 한다.
		JDBC에서는 기본이 AutoCOMMIT 모드이므로 수동모드로 전환 후 
		트랜잭션 처리를 해야 한다.
	...
	ex2) 카드 발급시에 포인트를 지급하기 작업단위가 설정.
			...
			1,2번 작업이 모두 정상적으로 수행되었을 때 실제 데이터베이스에 반영하도록
			구현해본다(COMMIT)
*/

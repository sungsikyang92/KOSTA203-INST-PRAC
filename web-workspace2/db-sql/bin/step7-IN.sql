...
SELECT*FROM product WHERE maker='롯데' OR maker='진로' OR maker='빙그레';

--위 sql을 IN으로 조회해본다
SELECT*FROM product WHERE maker in('롯데','진로','빙그레');

--MAKER가 롯데 진로 빙그레가 아닌 상품들을 조회
...
--1. GROUP BY를 이용해 MAKER 별 상품수를 확인
-- 크라운 1....
SELECT maker,COUNT(*)FROM product GROUP BY maker;

--2. GROUP BY조건절 HAVING을 이용해 상품수가 1개인 MAKER들만 조회
SELECT maker,COUNT(*)FROM product GROUP BY maker HAVING COUNT(*)=1;

--3. IN(SUBQUERY)를 이용해 상품 정보를 조회
SELECT*FROM product WHERE maker IN(SELECT maker FROM product GROUP BY maker HAVING COUNT(*)=1);
...
GROUP BY job HAVING AVG(salary)>(SELECT AVG(salary)
FROM employee);
--4) 그 JOB인 사원의 job과 name
SELECT job, name FROM employee 
WHERE job IN(SELECT job FROM employee 
GROUP BY job HAVING ROUND(AVG(salary))>(SELECT AVG(salary)
FROM employee));
